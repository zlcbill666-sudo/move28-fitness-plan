...
- 25 个审核动作均绑定器械组合、剂量、RPE、休息、安全提示和能力约束；当前25项均已使用本地动图库GIF开放。
...
参与者发布包必须严格按 [`release/runtime-manifest.json`](release/runtime-manifest.json) 的白名单构建。旧 `assets/gifs/`、未上架媒体研究区、`media-build/`、`media-src/`、研究资料、测试和开发脚本不得进入参与者包；参与者包只允许 `release/runtime-manifest.json` 白名单中的本地动图库GIF。
...
动作媒体来源和审核状态见 [docs/media-source-licenses.md](docs/media-source-licenses.md)；当前25项动作均已使用本地动图库GIF进入本地参与者发布包。跟练音乐来自 Mixkit Fitness Music，曲名与作者在播放器中显示；音乐加载失败只降级为静音，不影响训练步骤与安全停止。
