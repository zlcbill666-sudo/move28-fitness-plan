...
function saveTrack(){const d={};$$('#trackForm [data-label]').forEach(el=>d[el.dataset.label]=el.value);state.tracker[state.trackDay]=d;storage.setItem(state.storeKey,JSON.stringify(state.tracker));renderOverview();renderToday();$('#saveBtn').textContent='已保存 ✓';showToast(`第${state.trackDay}天已保存到当前浏览器`);setTimeout(()=>$('#saveBtn').textContent='保存今天 ✓',1400)}
