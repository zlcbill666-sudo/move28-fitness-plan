...
test('generated-plan 跟练严格消费session.actions，每屏一个动作并完成记录',async({page})=>{
  ...
  for(let index=0;index<expected.actions.length;index+=1){
    ...
    await expect(page.locator('#guideBody img[src^="assets/exercises/"]')).toHaveCount(1);
...
test('generated-plan 亚秒内点完18分钟5动作不写完成记录且保留固定恢复选择',async({page})=>{
  ...
  await page.getByRole('button',{name:'开始今天训练'}).click();await answerSafeReadiness(page);await page.getByRole('button',{name:'检查今天状态'}).click();await page.getByRole('button',{name:'按原计划继续'}).click();await page.getByRole('button',{name:'开始本节',exact:true}).click();
