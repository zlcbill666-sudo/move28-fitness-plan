...
test('pending_review 完成后手机首屏显示唯一等待状态、锁定原因和真实下一步', async ({ page }) => {
  ...
  const reviewSteps = pendingHero.locator('.hero-review-steps');
  const reviewMessage = pendingHero.locator('.review-message-sample');
  ...
  await expect(reviewSteps).toContainText('复制确认消息');
  await expect(reviewSteps).toContainText('下载确认文件');
  await expect(reviewSteps).toContainText('同一台设备和同一个浏览器');
  await expect(reviewMessage).toContainText('请帮我确认动作、器械和训练量是否适合我');
  ...
  await expectInsideViewport(reviewSteps, viewport);
