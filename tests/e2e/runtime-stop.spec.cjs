...
async function setup(page){await page.goto('/index.html');await page.evaluate(()=>{localStorage.clear();sessionStorage.clear()});await page.reload();await page.getByRole('button',{name:/生成我的4周计划/}).click();await page.evaluate(data=>{for(const [key,value]of Object.entries(data))Move28.onboardingController.setField(key,value);Move28.onboardingController.goTo(9)},safe);await page.locator('input[name="finalConfirmed"]').check();await page.getByRole('button',{name:/确认并保存结果/}).click();await page.getByRole('button',{name:'完成，返回首页'}).click();await page.evaluate(()=>{const key='move28-pilot-v1',state=JSON.parse(localStorage.getItem(key));state.plan.status='active';state.plan.review={status:'approved',reviewerId:'pilot-reviewer',reviewedAt:'2030-01-02T03:04:05.000Z',planId:state.plan.id,intakeRevision:state.intakeRevision};localStorage.setItem(key,JSON.stringify(state))});await page.reload();await page.getByRole('button',{name:'开始本节训练'}).click()}
