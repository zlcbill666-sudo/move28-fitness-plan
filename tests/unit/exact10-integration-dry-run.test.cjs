...
contract_data={'schemaVersion':1,'kind':'internal-media-integration-dry-run','candidateManifestSha256':hashlib.sha256(raw).hexdigest(),'productionPolicySha256':digest(runtime/'src/data/exercise-media-policy.js'),'formalManifestSha256':digest(runtime/'index.html'),'runtimeManifestSha256':digest(runtime/'runtime.json'),'runtimeFileSha256':{name:digest(runtime/pathlib.Path(*name.split('/'))) for name in manifest['files']},'approvedExerciseIds':ids,'expectedDryRun':{'released':10,'blocked':15,'formalReleaseEligible':25},'gates':{'motion':'approved-for-internal-candidate','visual':'approved-for-internal-preview','safety':'approved-for-internal-candidate','rights':'deferred-by-user-for-internal-stage','participantRelease':'exact10_allowlisted'},'forbiddenChanges':['src/data/exercise-media-policy.js','assets/exercises/manifest.json','release/runtime-manifest.json']};contract_path=runtime/'contract.json';contract_path.write_text(json.dumps(contract_data),encoding='utf-8')
...
localTest('真实隔离预演精确开放10项且生产已正式开放25项本地媒体',()=>{const result=run(['--verify']);assert.equal(result.status,0,result.stderr||result.stdout);const parsed=JSON.parse(result.stdout);assert.equal(parsed.released,10);assert.equal(parsed.blocked,15);assert.equal(parsed.participantRelease,'exact10_allowlisted');const report=JSON.parse(fs.readFileSync(path.join(defaultOutput,'dry-run-report.json'),'utf8'));assert.equal(report.formalReleaseEligible,25);const policy=loadPolicy(path.join(defaultOutput,'app/src/data/exercise-media-policy.js'));assert.equal(policy.releaseEligibleIds.length,10);assert.equal(policy.presentationFor('dead-bug').status,'blocked')});

localTest('真实Dashboard与跟练消费者行为显示Exact10媒体',()=>{
  const production=loadPolicy(path.join(root,'src/data/exercise-media-policy.js'));assert.equal(production.mode,'media_enabled');assert.equal(production.releaseEligibleIds.length,25);
