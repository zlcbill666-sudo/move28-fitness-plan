...
function setup(){
  ...
  const plan=generator.generatePlan({intake,risk,intakeRevision:1,catalog:catalog.exerciseCatalog});
  assert.equal(plan.status,'generated');
  return{catalog,generator,guide,app,plan};
}
test('plan-view active计划必须与当前intake revision一致并再次通过硬门',()=>{
  const {app,plan}=setup();
  const active={...structuredClone(plan),status:'active',review:{status:'approved',reviewerId:'pilot-reviewer',reviewedAt:'2030-01-02T03:04:05.000Z',planId:plan.id,intakeRevision:1}};
  let context=app.contextFromState({intake,intakeRevision:1,risk,plan:{...structuredClone(plan),status:'pending_review',review:null},logs:{}});
  assert.equal(context.mode,'review');
  context=app.contextFromState({intake,intakeRevision:1,risk,plan:active,logs:{}});
  assert.equal(context.mode,'generated');
  assert.deepEqual(context.plan,active);
  context=app.contextFromState({intake,intakeRevision:2,risk,plan:active,logs:{}});
  assert.equal(context.mode,'stale');
  const damaged=structuredClone(active);damaged.weeks[0].sessions[0].actions[0].reps=99;
  context=app.contextFromState({intake,intakeRevision:1,risk,plan:damaged,logs:{}});
  ...
  assert.doesNotThrow(()=>{context=app.contextFromState({intake,intakeRevision:1,risk,plan:hostilePlan,logs:{}})});
